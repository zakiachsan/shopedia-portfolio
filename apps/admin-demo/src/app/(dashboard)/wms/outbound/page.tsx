...
export default function OutboundPage() {
  const [txs] = useState(wmsTransactions.filter((t) => t.type === "outbound"))
      ...
      <Card>
        ...
        <CardContent>
          <div className="rounded-md bg-muted p-4 text-sm text-muted-foreground">
            In a real app, this form would record outbound stock. In demo mode, data is read-only.
          </div>
        </CardContent>
      </Card>
      ...
      >
        <Table>
          ...
          <TableBody>
            {txs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No outbound transactions.</TableCell>
              </TableRow>
            ) : (
              txs.slice().reverse().map((tx) => (
                <TableRow key={tx.id}>
                  ...
                  <TableCell className="text-right text-red-600 font-medium">{tx.delta}</TableCell>
